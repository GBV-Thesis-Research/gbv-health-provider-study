##################################################
## Project:
## Script purpose: Assessing Missingness
## Date:
## Author: 
##################################################
...
# Create new variables that show yes/no for missingness by domain
data_with_three_time_points$knowledgeMissing <- apply(
  data_with_three_time_points %>% select(starts_with("knowledge")),
  1,
  function(row) any(row == 99)
)

data_with_three_time_points$attitudeMissing <- apply(
  data_with_three_time_points %>% select(starts_with("attitude")),
  1,
  function(row) any(is.na(row))
)

data_with_three_time_points$confidenceMissing <- apply(
  data_with_three_time_points %>% select(starts_with("confidence")),
  1,
  function(row) any(is.na(row))
)

data_with_three_time_points$empathyMissing <- apply(
  data_with_three_time_points %>% select(starts_with("empathy")),
  1,
  function(row) any(is.na(row))
)

data_with_three_time_points$systemsupportMissing <- apply(
  data_with_three_time_points %>% select(starts_with("system")),
  1,
  function(row) any(is.na(row))
)

data_with_three_time_points$practicesMissing <- apply(
  data_with_three_time_points %>% select("practices_18"),
  1,
  function(row) any(is.na(row))
)

# Create new variables that tally missingness by domain
data_with_three_time_points$knowledgeMissingTally <- apply(
  data_with_three_time_points %>% select(starts_with("knowledge")),
  1,
  function(row) sum(row == 99)
)

data_with_three_time_points$attitudeMissingTally <- apply(
  data_with_three_time_points %>% select(starts_with("attitude")),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$confidenceMissingTally <- apply(
  data_with_three_time_points %>% select(starts_with("confidence")),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$empathyMissingTally <- apply(
  data_with_three_time_points %>% select(starts_with("empathy")),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$systemsupportMissingTally <- apply(
  data_with_three_time_points %>% select(starts_with("system")),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$practicesMissingTally <- apply(
  data_with_three_time_points %>% select("practices_18"),
  1,
  function(row) sum(is.na(row))
)

# Create new variables that tally demographic missingness
data_with_three_time_points$sex_factoredMissing <- apply(
  data_with_three_time_points %>% select("sex_factored"),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$position_groupsMissing <- apply(
  data_with_three_time_points %>% select("position_groups"),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$agegroupMissing <- apply(
  data_with_three_time_points %>% select("age_groups"),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$standardized_facilityMissing <- apply(
  data_with_three_time_points %>% select("standardized_facility"),
  1,
  function(row) sum(is.na(row))
)

data_with_three_time_points$position_years_cleanMissing <- apply(
  data_with_three_time_points %>% select("position_years_clean"),
  1,
  function(row) sum(is.na(row))
)

# Write to excel to check missingness
write.csv(data_with_three_time_points, file = "/Users/susanglenn/Desktop/School/data_with_three_time_points.csv")
